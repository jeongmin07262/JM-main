...
export default function BoardDetail() {
  ...
  const BoardDetailTop = () => {
    ...
    const onNicknameClickHandler = () => {
      if (!board) return;
        navigator(USER_PATH(board.writerNickname));
    }
    ...
    const onUpdateButtomClickHandler = () => {
    ...
    const onDeleteButtomClickHandler = () => {
        ...
        <div className='board-detail-top-header'>
          ...
          <div className='board-detail-top-sub-box'>
            ...
            <div className='board-detail-more-box'>
             <div className='board-detail-update-button' onClick={onUpdateButtomClickHandler}>{'수정'}</div>
             <div className='divider'></div>
             <div className='board-detail-delete-button' onClick={onDeleteButtomClickHandler}>{'삭제'}</div>
            </div>
            }
          </div>
        </div>
  ...
  const BoardDetailBottom = () => {
    ...
    //    state: 페이지네이션 관련 상태    //
    const 
     {currentPage,setCurrentPage ,currentSection, setCurrentSection ,viewList, viewPageList,totalSection,setTotalList} = usePagination<CommentListItem>(3);
    ...
    //    function: get favoirte list response 처리 함수    //
    const getFavoriteListResponse = (responseBody: GetFavoriteListResponseDto | ResponseDto | null) => {
      if (!responseBody) return;
      const { code } = responseBody;
      if (code === 'NB') alert('존재하지 않는 게시물입니다.');
      if (code === 'DBE') alert('존재하지 않은 데이터베이스입니다.');
      if (code !== 'SU') return;
    ...
    //    function: post comment response 처리함수    //
