...
import { GetSignInUserResponseDto, GetUserResponseDto, PatchNicknameResponseDto, PatchProfileImageResponseDto, DeleteUserResponseDto } from './response/user';
...
const DELETE_USER_URL = (email: string) => `${API_DOMAIN}/user/delete/${email}`;  
...
const PATCH_NICKNMAE_URL = () => `${API_DOMAIN}/user/nickname`;
const PATHC_PROFILE_IMAGE_URL = () => `${API_DOMAIN}/user/profile-image`;
...
export const patchNicknameRequest = async(requestBody: PatchNicknameRequestDto, accessToken: string) => {
    const result = await axios.patch(PATCH_NICKNMAE_URL(), requestBody, authorization(accessToken))
        .then(response => {
...
export const patchProfileImageRequest = async(requestBody: PatchProfileImageRequestDto, accessToken: string) => {
    const result = await axios.patch(PATHC_PROFILE_IMAGE_URL(), requestBody, authorization(accessToken))
        .then(response => {
